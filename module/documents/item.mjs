 ...
 * @extends {Item}
 */
export class AxisMundiRPGItem extends Item {
  /**
  ...
  async roll() {
    ...
    if (!item.system.formula || !item.system.formula.value) {
      ...
      let label = `<span class="chat-item-name">${game.i18n.localize('AXISMUNDIRPG.Roll')}: ${game.i18n.localize('ITEM.Type' + item.type.capitalize())} - ${item.name}</span>`;
