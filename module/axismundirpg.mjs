...
import { AxisMundiRPGActor } from './documents/actor.mjs';
import { AxisMundiRPGItem } from './documents/item.mjs';
// Import sheet classes.
import { AxisMundiRPGActorSheet } from './sheets/actor-sheet.mjs';
import { AxisMundiRPGItemSheet } from './sheets/item-sheet.mjs';
// Import helper/utility classes and constants.
import { preloadHandlebarsTemplates } from './helpers/templates.mjs';
import { AXISMUNDIRPG } from './helpers/config.mjs';
...
Hooks.once('init', async function() {
  ...
  CONFIG.AXISMUNDIRPG = AXISMUNDIRPG;
  ...
  CONFIG.Actor.documentClass = AxisMundiRPGActor;
  CONFIG.Item.documentClass = AxisMundiRPGItem;

  // Register sheet application classes
  Actors.unregisterSheet('core', ActorSheet);
  Actors.registerSheet('axismundirpg', AxisMundiRPGActorSheet, { makeDefault: true });
  Items.unregisterSheet('core', ItemSheet);
  Items.registerSheet('axismundirpg', AxisMundiRPGItemSheet, { makeDefault: true });
...
Handlebars.registerHelper('localizeItemNameForActor', function(type) {
  if (type === 'stronghold') {
    ...
    return game.i18n.localize('AXISMUNDIRPG.Cargo');
  } else {
...
Handlebars.registerPartial('iconDamage', `<i class="fa-solid fa-heart-crack fa-2xl" title="{{localize 'AXISMUNDIRPG.Roll'}} {{localize 'AXISMUNDIRPG.Damage'}}"></i>`);
//`<img src="systems/axismundirpg/styles/damage.svg" title="{{localize 'AXISMUNDIRPG.Roll'}} {{localize 'AXISMUNDIRPG.Damage'}}" width="24" height="24"/>`

Handlebars.registerPartial('iconMelee', `<i class="fa-solid fa-hand-fist fa-2xl" title="{{localize 'AXISMUNDIRPG.Roll'}} {{localize 'AXISMUNDIRPG.Melee'}} {{localize 'AXISMUNDIRPG.Attack'}}"></i>`);
//`<img src="systems/axismundirpg/styles/melee.svg" title="{{localize 'AXISMUNDIRPG.Roll'}} {{localize 'AXISMUNDIRPG.Melee'}} {{localize 'AXISMUNDIRPG.Attack'}}" width="24" height="24"/>`

Handlebars.registerPartial('iconRanged', `<i class="fa-solid fa-crosshairs fa-2xl" title="{{localize 'AXISMUNDIRPG.Roll'}} {{localize 'AXISMUNDIRPG.Ranged'}} {{localize 'AXISMUNDIRPG.Attack'}}"></i>`);
//`<img src="systems/axismundirpg/styles/ranged.svg" title="{{localize 'AXISMUNDIRPG.Roll'}} {{localize 'AXISMUNDIRPG.Ranged'}} {{localize 'AXISMUNDIRPG.Attack'}}" width="24" height="24"/>`
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.axismundirpg.rollItemMacro('${item.name}');`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'axismundirpg.itemMacro': true }
    });
  }
